...
\begin{description}
 ...
 \end{enumerate}
  ...
  \begin{example}
   ...
   \begin{itemize}
    \item \emph{VP}, sería predecir que una mujer está embarazada y que en efecto esté embarazada. \texbf{(Correcto)}
    \item \emph{VN}, sería con un hombre que no está embarazado y pues en efecto no está embarazado.\texbf{(Correcto)}  
    \item \emph{FP}, sería predecir que un hombre está embarazado y \texbf{no} este embarazado. \texbf{(Error, tipo 1)} 
    \item \emph{FN}, sería predecir que una mujer \texbf{no} está embarazada y esta embarazada. \texbf{(Error, tipo 2)}
   \end{itemize}
