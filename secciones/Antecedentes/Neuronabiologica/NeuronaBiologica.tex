...
La neurona es un tipo de célula perteneciente al sistema nervioso central, que se comunica tanto por señales eléctricas como por señales químicas. Cada neurona tiene \cite{sistemaNervioso}:%pag34
...
Como todas las neuronas pueden trasmitir o recibir información, para denotar claramente en que nos estamos enfocando para una neurona particual es necesario hacer las siguientes ditinciones:  
 \begin{itemize}
  \item Neurona presináptica, transmite una señal.
  \item Neurona postináptica, recibe una señal.
 \end{itemize}  

La transmisión de señales y almacenamiento de información en las neuronas se da de la siguiente forma:\cite{neurona_A_cerebro}
...
\subsection{Elementos de las neuronas}
...
\begin{itemize}
\item \textbf{Impulsos eléctricos:} potenciales de acción, cambios de voltaje que ocurrien a lo largo del axón. % Sucede una vez que se acumularon demasiadas señales a través de las dendritas, entonces la neurona puede disparar un impulso eléctrico, a través del axón, que va a provocar que su terminal libere más químicos, estos químicos son los que hacen los efectos pequeños en cada uno de los cuerpos de las neuronas postsinápticas.
...
\item \textbf{Neurotransmisor/es:} donde se encuentra la sustancia quimica, %\cite{sistemaNervioso pag.50.}
...
\subsection{Sinapsis}
 ...
 Donde dos neuronas en estrecha proximidad, se transmiten información se llama \textbf{sinapsis}. \cite{sistemaNervioso}%pag50.

 La sinapsis (típica) se establece entre el axon de una neurona y la dendrita de otra neurona.
...
\begin{description}
 \item [Sinapsis eléctrica:] las membranas de las células pre y postinápticas se unen en la brecha sináptica, que son pequeños canales que permiten el paso de iones (\fref{fig:sinapsisN}).
...
\item \textbf{Canales regulados por el voltaje:} Un voltaje, es el que abre o cierra el canal. Permite o no el paso del pulso electrico. % Existen una variante de este tipo, el cual tiene una pequeña compuerta abajo, que la puede cerrar independientemente.
\end{itemize}
...
La neurotransmision está detallada en la figura  \fref{fig:nTransmision}.
